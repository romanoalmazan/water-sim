...
const CameraStreamModal = ({ robotId, onClose, screenshotData }: CameraStreamModalProps) => {
  ...
  // Determine camera status based on light level
  const getCameraStatus = () => {
    if (!currentSegment) return { color: '#666', text: 'Unknown', isWarning: false }
    const light = currentSegment.Light
    if (light >= 0.5) {
      return { color: '#4ade80', text: 'OK', isWarning: false } // Green
    } else if (light >= 0.2) {
      return { color: '#fbbf24', text: 'Low Light', isWarning: false } // Yellow
    } else {
      return { color: '#ef4444', text: 'Warning', isWarning: true } // Red
    }
  }
          ...
          {currentSegment && (
            ...
            >
              ...
              <div>
                ...
                <span style={{ fontWeight: 'bold' }}>{robotId}</span>
              </div>
              <div>
